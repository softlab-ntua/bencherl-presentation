...
\begin{frame}[t]{Conclusions}
	\begin{itemize}
		\item \texttt{bencherl} is a publicly available, \textcolor{burgundy}{scalability} benchmark suite for Erlang/OTP
			...
			\end{itemize} 
		\item Examines how nodes, cores, schedulers, Erlang/OTP versions and \texttt{erl} command-line options affect the scalability of Erlang applications
		\item Collects scalability measurements
		\item Plots scalability graphs
	\end{itemize}
\end{frame}
...
\begin{frame}[t]{Future work}
	\begin{itemize}
		\item \texttt{bencherl} currently collects only execution times
			\begin{itemize}
				\item Collect more information during the execution of a benchmark (e.g. heap size)
			\end{itemize}
		\item \texttt{bencherl} currently can only answer to the question "Does this application scale well for this scenario?"
            \begin{itemize}
            	\item Try to answer questions like "Why doesn't this application scale well for this scenario?" 
			\end{itemize}
	\end{itemize}
\end{frame}
